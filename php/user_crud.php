...
if (isset($_POST['action']) && $_POST['action'] === 'update') {
    ...
    if (!empty($password)) {
        ...
        $stmt = $connect->prepare("UPDATE users SET userName=?, userPhone=?, userEmail=?, userRole=?, userStatus=?, userPassword=? WHERE userID=?"); 
        $stmt->bind_param("sssssss", $name, $phone, $email, $role, $status, $hashedPassword, $id); 
    } else {
        $stmt = $connect->prepare("UPDATE users SET userName=?, userPhone=?, userEmail=?, userRole=?, userStatus=? WHERE userID=?"); 
